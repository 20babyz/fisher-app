...
const CallHistoryScreen: React.FC = () => {
  ...
  const renderItem = ({ item }: { item: HistoryItem }) => {
    ...
    switch (item.callType) {
      case 'incoming':
        iconName = 'arrow-down-circle'; iconColor = '#4CAF50'; break;
      case 'outgoing':
        iconName = 'arrow-up-circle';   iconColor = '#007AFF'; break;
      case 'missed':
        iconName = 'close-circle';       iconColor = '#FF3B30'; break;
    }

    return (
      <TouchableOpacity
        style={styles.itemContainer}
        onPress={() => navigation.navigate('Profile', { contact: { id: item.id, name: item.name, lastMessage: item.summary, unread: 0 } })}
      >
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerRow}>
        <TouchableOpacity onPress={() => navigation.navigate('EditRisk', { contact: undefined as any, riskNumbers: [] })}>
...
const styles = StyleSheet.create({
  ...
  searchBar:    { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F5F7FA', margin: 16, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6 },
  searchInput:  { flex: 1, marginLeft: 8 },

  list:         { paddingHorizontal: 16 },
  sectionHeader:{ marginTop: 16, marginBottom: 8, fontSize: 14, fontWeight: '500', color: '#666' },
